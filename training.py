...
# Set the backend for matplotlib to avoid displaying issues in some environments
matplotlib.use('Agg')
...
def train_step(state: train_state.TrainState, batch, rng):
    ...
    logits = state.apply_fn({"params": state.params}, batch["inputs"], rng=rng)  # Pass the RNG here
...
def eval_step(state: train_state.TrainState, batch, rng):
    ...
    rng, rng_key = jax.random.split(rng)
    logits = state.apply_fn({"params": state.params}, batch["inputs"], rng=rng_key)  # Pass the RNG here
    return jnp.argmax(logits, axis=-1), rng

def evaluate_model(state, texts, labels, rng):
    predictions, rng = eval_step(state, {"inputs": texts}, rng)  # Pass the RNG to eval_step
    accuracy = calculate_accuracy(predictions, jnp.argmax(labels, axis=-1))
    return accuracy, rng

def print_predictions(state, inputs, labels, rng):
    predictions, rng = eval_step(state, {"inputs": inputs}, rng)  # Pass the RNG to eval_step
...
def create_train_state(rng, model, num_classes):
    ...
    params = model.init(rng, jnp.ones((1, 100)))["params"]  # Adjust input shape as necessary
...
def main():
    ...
    for epoch in tqdm(range(num_epochs), desc="Training Epochs", unit="epoch"):
        ...
        state, loss_value, accuracy, rng = train_step(state, batch, rng)  # Pass rng to train_step
        losses.append(loss_value)
        accuracies.append(accuracy)

        test_labels_one_hot = one_hot(test_labels, num_classes=2)
        test_accuracy, rng = evaluate_model(state, test_inputs, test_labels_one_hot, rng)  # Pass rng to evaluate_model
        test_accuracies.append(test_accuracy)

        print(f"Loss: {loss_value:.4f}, Accuracy: {accuracy:.4f}, Test Accuracy: {test_accuracy:.4f}")

        # Save model parameters
        with open("model_params.npz", "wb") as f:
            jax.numpy.savez(f, params=state.params)

    # Uncomment the following lines to plot training loss and accuracies
    ...
    print_predictions(state, test_inputs, jnp.argmax(test_labels_one_hot, axis=-1), rng)  # Pass rng to print_predictions
    plot_confusion_matrix(state, test_inputs, jnp.argmax(test_labels_one_hot, axis=-1), rng)  # Pass rng to plot_confusion_matrix
